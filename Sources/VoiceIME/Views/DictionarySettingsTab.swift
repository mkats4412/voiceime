...
public struct DictionarySettingsTab: View {
    ...
    @State private var testInput: String = "マックとウインドウズを使って作業します。改行よろしくお願いいたします。"
                ...
                VStack(alignment: .leading, spacing: 12) {
                    HStack(alignment: .top) {
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            Text("音声入力完了後、略称を正式名称に展開したり、改行などのコマンドに強制変換します。\n(例: \"マック\" ➔ \"Apple macOS\"、\"ウインドウズ\" ➔ \"Microsoft Windows\"、\"改行\" ➔ \"\\n\")")
                                .font(.caption)
                                .foregroundColor(.secondary)
                                .fixedSize(horizontal: false, vertical: true)
                        }
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        HStack(spacing: 8) {
                            TextField("置換前 (例: マック)", text: $newPattern)
                                .textFieldStyle(.roundedBorder)
